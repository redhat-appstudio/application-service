...
var _ = Describe("SnapshotEnvironmentBinding controller", func() {
	...
	Context("Create SnapshotEnvironmentBinding with a missing component", func() {
		It("Should fail if there is no such component by name", func() {
			...
			Expect(createdBinding.Status.GitOpsRepoConditions[len(createdBinding.Status.GitOpsRepoConditions)-1].Message).Should(ContainSubstring(fmt.Sprintf("GitOps repository sync failed: unable to get the Component %s", componentName2)))
